...
SELECT 
    ...
    p.property_type,
    p.assessed_value::FLOAT as assessed_value,
    p.flood_zone,
    t.tax_amount::FLOAT as tax_amount,
    tj.tax_rate::FLOAT as tax_rate,
    tj.jurisdiction_type,
    t.penalty_amount::FLOAT as penalty_amount,
    DATEDIFF('day', t.due_date, CURRENT_DATE())::FLOAT AS days_since_due,
    DATEDIFF('day', COALESCE(t.payment_date, CURRENT_DATE()), CURRENT_DATE())::FLOAT AS days_since_last_payment,
    l.loan_type,
    l.loan_amount::FLOAT as loan_amount,
    l.escrow_account,
    l.loan_status,
    c.client_type,
    c.service_quality_score::FLOAT as service_quality_score,
    c.client_status,
...
WITH client_metrics AS (
    SELECT 
        ...
        c.client_type,
        c.service_quality_score::FLOAT as service_quality_score,
        c.total_properties::FLOAT as total_properties,
        c.lifetime_value::FLOAT as lifetime_value,
        DATEDIFF('month', c.onboarding_date, CURRENT_DATE())::FLOAT AS months_as_client,
        s.service_type,
        s.subscription_tier,
        s.billing_cycle,
        ...
        AVG(st.resolution_time_hours)::FLOAT AS avg_resolution_time,
...
SELECT 
    ...
    l.loan_status,
    l.escrow_account,
    DATEDIFF('month', l.loan_date, CURRENT_DATE())::FLOAT AS loan_age_months,
    (l.loan_amount / NULLIF(p.assessed_value, 0))::FLOAT AS loan_to_value_ratio,
    p.property_type,
    p.assessed_value::FLOAT as assessed_value,
    p.flood_zone,
    p.property_state,
    ...
    DATEDIFF('day', t.due_date, COALESCE(t.payment_date, CURRENT_DATE()))::FLOAT AS days_payment_delay,
    c.client_type,
    c.service_quality_score::FLOAT as service_quality_score,
